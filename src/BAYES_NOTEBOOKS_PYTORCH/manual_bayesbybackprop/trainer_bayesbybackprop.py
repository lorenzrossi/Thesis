"""
Training and testing module for Bayesian energy forecasting models using manual BayesByBackprop.

This module uses the manual implementation of BayesByBackprop (models_bayesbybackprop.py)
instead of the blitz library, providing full control and transparency over the algorithm.
"""
...
# Import manual BayesByBackprop models
...
from models_bayesbybackprop import (
    create_model, 
    ModelBuilder, 
    DEFAULT_CONFIGS,
    compute_elbo_loss,
...
class BayesianModelTrainer:
    ...
    def __init__(
        ...
        n_samples: int = 5,  # Number of samples for ELBO during training
        ...
        **model_kwargs
        ...
        """
        Initialize Bayesian model trainer using manual BayesByBackprop.
        ...
        n_samples : int
            Number of Monte Carlo samples for ELBO during training
    ...
    def _train_epoch(
        ...
        """
        Train for one epoch using ELBO loss (manual BayesByBackprop).
        
        Returns:
        --------
        elbo, likelihood, kl_div : Tuple of average losses
        """
        model.train()
        elbo_total = 0.0
        ...
        for datapoints, labels in dataloader:
            ...
            # Compute ELBO loss using manual implementation
            ...
            likelihood_total += likelihood.item()
            kl_div_total += kl_div.item()
            n_batches += 1
        
        return (
            elbo_total / n_batches if n_batches > 0 else 0.0,
            likelihood_total / n_batches if n_batches > 0 else 0.0,
    ...
    def train(
        ...
        if self.verbose:
            print(f"\n{'='*60}")
            print(f"Training Bayesian MLP (Manual BayesByBackprop) with {self.feature_type} features")
        ...
        for i in test_iterations:
            ...
            for epoch in range(self.epochs):
                elbo, likelihood, kl_div = self._train_epoch(
                    self.model, dataloader, criterion, optimizer, len(history_X)
                )
                
                if self.verbose and (epoch + 1) % max(1, self.epochs // 5) == 0:
                    print(f"Epoch {epoch+1}/{self.epochs} - ELBO: {elbo:.6f}, Likelihood: {likelihood:.6f}, KL Div: {kl_div:.6f}")
        ...
        # Store results
        self.results = {
            'model_type': 'bayesian_mlp_manual',
        ...
        if save_model:
            if model_path is None:
                model_path = f"bayesian_mlp_manual_{self.feature_type}_model.pth"
            torch.save({
...
if __name__ == "__main__":
    # Example usage
    print("Manual BayesByBackprop Trainer")
    print("="*60)
    print("This trainer uses the manual implementation of BayesByBackprop")
    print("instead of the blitz library, providing full transparency.")
    print("="*60)
