"""
Training and testing module for energy forecasting models using PyTorch.
...
class ModelTrainer:
    ...
    def __init__(
        ...
        **model_kwargs
        ...
        device : str, optional
            Device to use ('cpu', 'cuda', or None for auto-detect)
        **model_kwargs
    ...
    def _train_epoch(self, model: nn.Module, dataloader: DataLoader, criterion: nn.Module, optimizer: optim.Optimizer) -> float:
        """Train for one epoch."""
        model.train()
        total_loss = 0.0
        n_batches = 0
    ...
    def train(
        ...
        # Align truth with y_test positions
        # For MLP: y_test is created after dropping NaN from lags, so we need to account for that
        # For LSTM/CNN: y_test starts from window_size position in test_data
        if self.model_type == 'mlp':
            # For MLP, y_test corresponds to test_data positions after window_size
            # The first window_size rows are dropped due to NaN from lags
            # So truth should start from window_size position in test_data_raw
            ...
            # For LSTM/CNN, y_test starts from window_size position
            # So truth should start from window_size position in test_data_raw
            truth_start_idx = self.window_size
            truth = test_data_raw[truth_start_idx:truth_start_idx + len(y_test), 0] if len(y_test) > 0 else test_data_raw[truth_start_idx:, 0]
        ...
        if self.model_type == 'mlp':
            ...
            input_size = X_train.shape[2] if X_train.ndim > 2 else 1
        ...
        # Create model
        self.model = create_model(self.model_type, input_size, **config)
        ...
        if len(list(test_iterations)) == 0:
            ...
            # Align truth with y_test positions (same logic as above)
            if self.model_type == 'mlp':
                truth_start_idx = self.window_size
                truth = train_data_raw[truth_start_idx:truth_start_idx + len(y_test), 0] if len(y_test) > 0 else train_data_raw[truth_start_idx:, 0]
            else:
                truth_start_idx = self.window_size
                truth = train_data_raw[truth_start_idx:truth_start_idx + len(y_test), 0] if len(y_test) > 0 else train_data_raw[truth_start_idx:, 0]
        ...
        for i in test_iterations:
            ...
            X_tensor = torch.FloatTensor(history_X)
            y_tensor = torch.FloatTensor(history_y)
            ...
            for epoch in range(self.epochs):
                train_loss = self._train_epoch(self.model, dataloader, criterion, optimizer)
                
                # Check early stopping
                if early_stopping(train_loss, self.model):
                    if self.verbose:
                        print(f"Early stopping at epoch {epoch + 1}")
                    break
            
            # Make predictions
            yhat = self._predict(self.model, X_test_block)
        ...
        if save_model:
            if model_path is None:
                model_path = f"{self.model_type}_{self.feature_type}_model.pth"
            torch.save({
...
def run_all_models(
    ...
    if save_results:
        ...
        results_file = os.path.join(results_path, "model_comparison_pytorch.csv")
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description='Train energy forecasting models with PyTorch')
